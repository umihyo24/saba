import { CONFIG, ASSETS, DIRS } from "./core/config.js";
import { gameState } from "./core/state.js";
import { ITEM_DEFS, CRAFT_RECIPES } from "./data/items.js";
import { ENEMY_TYPES } from "./data/enemies.js";
import { DUNGEON_DEFS, DUNGEON_THEMES, DEFAULT_DISCOVERY_FOV } from "./data/dungeons.js";
import { createObjectTypes } from "./data/objects.js";
import { renderMiniMap as renderMiniMapView } from "./render/renderMiniMap.js";
import { renderArea as renderAreaView } from "./render/renderArea.js";
import { renderHudBar as renderHudBarView, renderMessageBox as renderMessageBoxView, renderStatusPanel as renderStatusPanelView, missionHintText as missionHintTextView } from "./render/renderHud.js";
import { bindUiEvents } from "./ui/actions.js";
import { debugUi, debugFlow, debugClickCapture } from "./ui/debug.js";
...
const OBJECT_TYPES = createObjectTypes({ gameState, addLog, startDungeonRun, descendDepth, onNpcRescued, endPlayerTurn });
...
function renderMiniMap(area, cam) {
  return renderMiniMapView(area, gameState, { tileAt, isDiscovered });
}

function renderArea(area, hint, overlays = "") {
  return renderAreaView(area, hint, overlays, { cameraFor, renderBoardStage });
}

function renderHudBar() {
  renderHudBarView({ hudEl, gameState, getDungeonDef });
}

function renderMessageBox() {
  renderMessageBoxView({ logEl, gameState, logLimit: CONFIG.logLimit });
}

function renderStatusPanel() {
  return renderStatusPanelView({
    gameState,
    lungCapacity: CONFIG.lungCapacity,
    carryingWeight,
    carryWeightLimit: CONFIG.carryWeightLimit,
  });
}

function missionHintText() {
  return missionHintTextView(gameState);
}
...
bindUiEvents({ viewEl, windowObj: window, gameState, DIRS, dispatch, render, debugClickCapture });
